<?php
	...
	if($cmd == 'info' && $id && $idmuc)
	{
		...
		if($d->update('gallery',$data))
		{
			...
			{
				echo galleryFiler($gallery[$i]['stt'],$gallery[$i]['id'],$gallery[$i]['photo'],$gallery[$i]['tenvi'],$com,$colfiler);
			}
		}
	}
	...
	{
		$row = $d->rawQueryOne("select photo, thumb from #_gallery where id = ?",array($id));

		$path="../../upload/".$folder."/".$row['photo'];
		$path_thumb="../../upload/".$folder."/".$row['thumb'];
	...
	else if($cmd == 'delete-all' && $listid)
	{
		...
		for($i=0;$i<count($row);$i++)
		{
			...
			$path_thumb="../../upload/".$folder."/".$row[$i]['thumb'];
