<?php
	...
	function createCopy($titleCopy,$titleSlug,$table)
	{
		...
		{
			$titleCopy .= " (1)";
			$titleSlug = changeTitle($titleCopy);
		...
		else
		{
			foreach($config['website']['lang'] as $key => $value) 
			{
				...
				if($table=='product') $datacopy['khuyenmai'.$key] = $item['khuyenmai'.$key];
			}
			$datacopy['tenvi'] = $titleCopy;
			$datacopy['tenkhongdauvi'] = changeTitle($datacopy['tenvi']);
