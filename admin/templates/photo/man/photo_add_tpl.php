...
<section class="content">
    <form method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
		...
		<?php $numberPhoto = $config['photo']['man_photo'][$type]['number_photo']; ?>
		<?php for($i=0;$i<$numberPhoto;$i++) { $stt = $i+1; ?>
			<div class="card card-primary card-outline text-sm">
	            ...
	            <div class="card-body">
					<?php if($config['photo']['man_photo'][$type]['images_photo']) { ?>
	                ...
	                <?php } ?>
	                <?php if($config['photo']['man_photo'][$type]['mausac_photo']) { ?>
		            ...
		            <?php if($config['photo']['man_photo'][$type]['tieude_photo'] || $config['photo']['man_photo'][$type]['mota_photo'] || $config['photo']['man_photo'][$type]['noidung_photo']) { ?>
		                        ...
		                        <div class="tab-content" id="custom-tabs-three-tabContent-lang">
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>-<?=$i?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php } ?>
		                                    <?php if($config['photo']['man_photo'][$type]['mota_photo']) { ?>
		                                        <div class="form-group">
		                                            <label for="mota<?=$k?><?=$i?>">Mô tả (<?=$k?>):</label>
		                                            <textarea class="form-control <?=($config['photo']['man_photo'][$type]['mota_cke_photo'])?'form-control-ckeditor':''?>" name="dataMulti[<?=$i?>][mota<?=$k?>]" id="mota<?=$k?><?=$i?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode($item['mota'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if($config['photo']['man_photo'][$type]['noidung_photo']) { ?>
		                                        <div class="form-group">
		                                            <label for="noidung<?=$k?><?=$i?>">Nội dung (<?=$k?>):</label>
		                                            <textarea class="form-control <?=($config['photo']['man_photo'][$type]['noidung_cke_photo'])?'form-control-ckeditor':''?>" name="dataMulti[<?=$i?>][noidung<?=$k?>]" id="noidung<?=$k?><?=$i?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode($item['noidung'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
