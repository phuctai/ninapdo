                                    ...
                                    <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                        ...
                                        <?php if($config['photo']['man_photo'][$type]['mota_photo']) { ?>
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control <?=($config['photo']['man_photo'][$type]['mota_cke_photo'])?'form-control-ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode($item['mota'.$k])?></textarea>
                                            </div>
                                        <?php } ?>
                                        <?php if($config['photo']['man_photo'][$type]['noidung_photo']) { ?>
                                            <div class="form-group">
                                                <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                                <textarea class="form-control <?=($config['photo']['man_photo'][$type]['noidung_cke_photo'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode($item['noidung'.$k])?></textarea>
                                            </div>
                                        <?php } ?>
                                    </div>
                                <?php } ?>
