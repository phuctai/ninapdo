<?php
    ...
    if($config['news'][$type]['images_list'] || $flagGallery)
    {
        $colLeft = "col-xl-8";
        $colRight = "col-xl-4";
    }
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if($config['news'][$type]['mota_list']) { ?>
                                                <div class="form-group">
                                                    <label for="mota<?=$k?>">Mô tả (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['news'][$type]['mota_cke_list'])?'form-control-ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode($item['mota'.$k])?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if($config['news'][$type]['noidung_list']) { ?>
                                                <div class="form-group">
                                                    <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['news'][$type]['noidung_cke_list'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode($item['noidung'.$k])?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
            ...
            <div class="<?=$colRight?>">
                <?php if($config['news'][$type]['images_list']) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
                                ...
                                <div class="form-group form-group-gallery">
                                    ...
                                    <div class="jFiler-items my-jFiler-items jFiler-row">
                                        <ul class="jFiler-items-list jFiler-items-grid row scroll-bar" id="jFilerSortable">
                                            <?php foreach($gallery as $v) galleryFiler($v['stt'],$v['id'],$v['photo'],$v['tenvi'],'news','col-xl-6 col-sm-3 col-6'); ?>
                                        </ul>
                                    </div>
                                </div>
                            <?php } ?>
        ...
        <?php if($config['news'][$type]['seo_list']) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
