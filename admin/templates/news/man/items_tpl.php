            ...
            <table class="table table-hover">
                <thead>
                    <tr>
						...
						<?php if($config['news'][$type]['show_images']) { ?>
							<th class="align-middle">Hình</th>
						<?php } ?>
						<th class="align-middle" style="width:30%">Tiêu đề</th>
						<?php if(count($config['news'][$type]['gallery'])) { ?>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <?php if($config['news'][$type]['show_images']) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['tenvi']?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?=UPLOAD_NEWS.$items[$i]['photo']?>" alt="<?=$items[$i]['tenvi']?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                    <a class="text-dark" href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['tenvi']?>"><?=$items[$i]['tenvi']?></a>
                                    <div class="tool-action mt-2 w-clear">
                                    	<?php if($config['news'][$type]['view']) { ?>
                                    		<a class="text-primary mr-3" href="<?=$linkView?><?=$func->get_slug('vi',$items[$i]['id'],'news')?>" target="_blank" title="<?=$items[$i]['tenvi']?>"><i class="far fa-eye mr-1"></i>View</a>
                                    	<?php } ?>
                                    	<a class="text-info mr-3" href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['tenvi']?>"><i class="far fa-edit mr-1"></i>Edit</a>
                                    	<?php if($config['news'][$type]['copy']) { ?>
                                    		<div class="dropdown">
