<?php
    ...
    function getPayments()
    {
        ...
        $row = $d->rawQuery("select * from table_news where type='hinh-thuc-thanh-toan' order by stt,id desc");
    ...
    function getCity()
    {
        ...
        $row = $d->rawQuery("select ten,id from table_city order by id asc");

        $str = '<select id="city" name="id_city" data-level="0" data-table="table_district" data-child="district" class="form-control select2 select-place"><option value="">Chọn danh mục</option>';
        foreach($row as $v)
        {
    ...
    function getDistrict()
    {
        ...
        $row = $d->rawQuery("select ten,id from table_district where id_city = ? order by id asc",array($id_city));

        $str = '<select id="district" name="id_district" data-level="1" data-type="'.$type.'" data-table="table_wards" data-child="wards" class="form-control select2 select-place"><option value="">Chọn danh mục</option>';
        foreach($row as $v)
        {
    ...
    function getWards()
    {
        ...
        $row = $d->rawQuery("select ten,id from table_wards where id_city = ? and id_district = ? order by id asc",array($id_city,$id_district));

        $str = '<select id="wards" name="id_wards" class="form-control select2"><option value="">Chọn danh mục</option>';
        foreach($row as $v)
        {
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <span class="text-info"><?=get_payments($items[$i]['httt'])?></span>
                                </td>
                                <td class="align-middle">
                                ...
                                <td class="align-middle">
                                    <?php
                                        ...
                                        $tinhtrang = $d->rawQueryOne("SELECT trangthai FROM table_status WHERE id = ?",array($id_tinhtrang));
                                    ?>
