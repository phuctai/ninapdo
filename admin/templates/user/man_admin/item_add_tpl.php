<?php
	if($config['permission'])
	{
		function get_nhomquyen($id_nhomquyen)
		{
			...
			$row = $d->rawQuery("select * from table_permission_group where hienthi>0 order by stt,id desc");

			$str = '<select id="id_nhomquyen" name="data[id_nhomquyen]" class="form-control select2"><option value="">Nhóm quyền</option>';
			foreach($row as $v)
			{
