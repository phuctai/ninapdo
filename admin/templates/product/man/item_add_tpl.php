<?php
	function get_main_list()
    {
        ...
        $str = '<select id="id_list" name="data[id_list]" data-level="0" data-type="'.$type.'" data-table="table_product_cat" data-child="id_cat" class="form-control select2 select-category"><option value="">Chọn danh mục</option>';
        foreach($row as $v)
        {
	...
	function get_main_cat()
	{
		...
		$str = '<select id="id_cat" name="data[id_cat]" data-level="1" data-type="'.$type.'" data-table="table_product_item" data-child="id_item" class="form-control select2 select-category"><option value="">Chọn danh mục</option>';
		foreach($row as $v)
        {
	...
	function get_main_item()
	{
		...
		$str = '<select id="id_item" name="data[id_item]" data-level="2" data-type="'.$type.'" data-table="table_product_sub" data-child="id_sub" class="form-control select2 select-category"><option value="">Chọn danh mục</option>';
		foreach($row as $v)
        {
	...
	function get_main_brand()
    {
        ...
        $str = '<select id="id_brand" name="data[id_brand]" class="form-control select2"><option value="">Danh mục hãng</option>';
        foreach($row as $v)
        {
	...
	function get_tags($id="")
	{
		...
		if($id)
		{
			$temps = $d->rawQueryOne("select id_tags from table_product where id = ? and type = ?",array($id,$type));
	...
	function get_mau($id="")
	{
		...
		if($id)
		{
			$temps = $d->rawQueryOne("select id_mau from table_product where id = ? and type = ?",array($id,$type));
			$arr_mau = explode(',', $temps['id_mau']);
			
			for($i=0;$i<count($arr_mau);$i++) $temp[$i]=$arr_mau[$i];
		}

		$row_mau = $d->rawQuery("select tenvi, id from table_product_mau where type = ? order by stt,id desc",array($type));
	...
	function get_size($id="")
	{
		...
		if($id)
		{
			$temps = $d->rawQueryOne("select id_size from table_product where id = ? and type = ?",array($id,$type));
			$arr_size = explode(',', $temps['id_size']);
			
			for($i=0;$i<count($arr_size);$i++) $temp[$i]=$arr_size[$i];
		}

		$row_size = $d->rawQuery("select tenvi, id from table_product_size where type = ? order by stt,id desc",array($type));
    ...
    if($config['product'][$type]['dropdown'] || $config['product'][$type]['brand'] || $config['product'][$type]['tags'] || $config['product'][$type]['mau'] || $config['product'][$type]['size'] || $config['product'][$type]['images'] || $flagGallery)
    {
    	$colLeft = "col-xl-8";
    	$colRight = "col-xl-4";
    }
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
		                        ...
		                        <div class="tab-content" id="custom-tabs-three-tabContent-lang">
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php if($config['product'][$type]['khuyenmai']) { ?>
		                                        <div class="form-group">
		                                            <label for="khuyenmai<?=$k?>">Khuyến mãi (<?=$k?>):</label>
		                                            <textarea class="form-control <?=($config['product'][$type]['khuyenmai_cke'])?'form-control-ckeditor':''?>" name="data[khuyenmai<?=$k?>]" id="khuyenmai<?=$k?>" rows="5" placeholder="Khuyến mãi (<?=$k?>)"><?=htmlspecialchars_decode($item['khuyenmai'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if($config['product'][$type]['mota']) { ?>
		                                        <div class="form-group">
		                                            <label for="mota<?=$k?>">Mô tả (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=($config['product'][$type]['mota_cke'])?'form-control-ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode($item['mota'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if($config['product'][$type]['noidung']) { ?>
		                                        <div class="form-group">
		                                            <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=($config['product'][$type]['noidung_cke'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode($item['noidung'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
        	...
        	<div class="<?=$colRight?>">
        		<?php if($config['product'][$type]['dropdown'] || $config['product'][$type]['brand'] || $config['product'][$type]['tags'] || $config['product'][$type]['mau'] || $config['product'][$type]['size']) { ?>
			        <div class="card card-primary card-outline text-sm">
			            <div class="card-header">
			            ...
			            <div class="card-body">
		            		<div class="form-group-category row">
				            	<?php if($config['product'][$type]['dropdown']) { ?>
				...
				<?php if($config['product'][$type]['images']) { ?>
			        <div class="card card-primary card-outline text-sm">
			            <div class="card-header">
	    ...
	    <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
            	...
            	<?php if($config['product'][$type]['file']) { ?>
                    <div class="form-group">
                ...
                <div class="row">
	                ...
	                <?php } ?>
					<?php if($config['product'][$type]['gia']) { ?>
				    	<div class="form-group col-md-4">
				    ...
				    <?php } ?>
				    <?php if($config['product'][$type]['giamoi']) { ?>
				    	<div class="form-group col-md-4">
				    ...
				    <?php } ?>
				    <?php if($config['product'][$type]['giakm']) { ?>
				    	<div class="form-group col-md-4">
	                ...
	                <?php } ?>
				    <?php if($config['product'][$type]['tinhtrang']) { ?>
					    <div class="form-group col-md-4">
        		...
        		<?php if($config['product'][$type]['attribute']) { $addAttribute = true; ?>
        ...
        <?php if($config['product'][$type]['seo']) { ?>
			<div class="card card-primary card-outline text-sm">
	            <div class="card-header">
