<?php
    ...
    /* Check cols */
    if($config['product'][$type]['images_brand'])
    {
        $colLeft = "col-xl-8";
        $colRight = "col-xl-4";
    }
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if($config['product'][$type]['mota_brand']) { ?>
                                                <div class="form-group">
                                                    <label for="mota<?=$k?>">Mô tả (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['product'][$type]['mota_cke_brand'])?'form-control-ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode($item['mota'.$k])?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if($config['product'][$type]['noidung_brand']) { ?>
                                                <div class="form-group">
                                                    <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['product'][$type]['noidung_cke_brand'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode($item['noidung'.$k])?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
            ...
            <div class="<?=$colRight?>">
                <?php if($config['product'][$type]['images_brand']) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
        ...
        <?php if($config['product'][$type]['seo_brand']) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
                ...
                <div class="card-body">
                    <?php
                        $seo = $func->get_seo($id,$com,'man_brand',$type);
