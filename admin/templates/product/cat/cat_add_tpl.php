<?php
	function get_main_list()
    {
        ...
        $str = '<select id="id_list" name="data[id_list]" class="form-control select2"><option value="">Chọn danh mục</option>';
        foreach($row as $v)
        {
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-xl-8">
                <?php
                if($config['product'][$type]['slug_cat'])
                {
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if($config['product'][$type]['mota_cat']) { ?>
                                                <div class="form-group">
                                                    <label for="mota<?=$k?>">Mô tả (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['product'][$type]['mota_cke_cat'])?'form-control-ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode($item['mota'.$k])?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if($config['product'][$type]['noidung_cat']) { ?>
                                                <div class="form-group">
                                                    <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                                    <textarea class="form-control for-seo <?=($config['product'][$type]['noidung_cke_cat'])?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode($item['noidung'.$k])?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
            ...
            <div class="col-xl-4">
                ...
                <?php if($config['product'][$type]['images_cat']) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
        ...
        <?php if($config['product'][$type]['seo_cat']) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
                ...
                <div class="card-body">
                    <?php
                        $seo = $func->get_seo($id,$com,'man_cat',$type);
