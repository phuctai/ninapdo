            ...
            <table class="table table-hover">
                <thead>
                    <tr>
						...
						<?php if($config['product'][$type]['show_images_list']) { ?>
							<th class="align-middle">Hình</th>
						<?php } ?>
						<th class="align-middle" style="width:30%">Tiêu đề</th>
						<?php if(count($config['product'][$type]['gallery_list'])) { ?>
                            <th class="align-middle">Gallery</th>
                        <?php } ?>
						<?php foreach($config['product'][$type]['check_list'] as $key => $value) { ?>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <?php if($config['product'][$type]['show_images_list']) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['tenvi']?>"><img class="rounded img-preview" onerror="src='assets/images/noimage.png'" src="<?=_upload_product.$items[$i]['photo']?>" alt="<?=$items[$i]['tenvi']?>"></a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
