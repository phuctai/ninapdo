...
function save_seopage()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=seopage&act=capnhat&type=".$type,0);
	...
	{
		$dataSeo['photo'] = $photo;
		$dataSeo['thumb'] = $func->createThumb($dataSeo['photo'], $config['seopage']['thumb_width'], $config['seopage']['thumb_height'], UPLOAD_SEOPAGE,$file_name,$config['seopage']['thumb_ratio']);
		
		$row = $d->rawQueryOne("select id, photo, thumb from #_seopage where type = ?",array($type));

		if($row['id'])
		{
			$func->delete_file(UPLOAD_SEOPAGE.$row['photo']);
			$func->delete_file(UPLOAD_SEOPAGE.$row['thumb']);
		}
	}
	...
	if($seopage['id'])
	{
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=seopage&act=capnhat&type=".$type,0);
	}
	else
	{
		if($d->insert('seopage',$dataSeo)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=seopage&act=capnhat&type=".$type);
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=seopage&act=capnhat&type=".$type,0);
	}
}
?>