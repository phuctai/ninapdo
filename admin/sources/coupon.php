...
if(!$config['coupon']) $func->transfer("Trang không tồn tại", "index.php",0);
...
function get_item()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=coupon&act=man&p=".$curPage,0);

	$item = $d->rawQueryOne("select * from #_coupon where id = ?",array($id));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=coupon&act=man&p=".$curPage,0);
}
...
function save_item()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=coupon&act=man&p=".$curPage,0);
	...
	if($id)
	{
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=coupon&act=man&p=".$curPage,0);
	}
	else
	{
		...
		if($quanitycode)
		{
			for($i=0;$i<$quanitycode;$i++)
			{
				...
				if(!$d->insert('coupon',$data)) $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=coupon&act=man&p=".$curPage,0);
			}
		...
		{
			$func->transfer("Dữ liệu rỗng", "index.php?com=coupon&act=man&p=".$curPage,0);
		}
	}
}
...
function delete_item()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=coupon&act=man&p=".$curPage,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=coupon&act=man&p=".$curPage,0);
}
?>