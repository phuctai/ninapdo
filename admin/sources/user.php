...
switch($act)
{
	...
	case "login":
		if(isset($_SESSION[$login_admin]) && $_SESSION[$login_admin] == true) $func->transfer("Trang không tồn tại", "index.php", false);
	...
	case "man_admin":
		...
		if(!$config['user']['admin']) $func->transfer("Trang không tồn tại", "index.php", false);
	...
	case "man":
		...
		if(!$config['user']['visitor']) $func->transfer("Trang không tồn tại", "index.php", false);
		if($func->check_permission())
		{
			$func->transfer("Bạn không có quyền vào trang này", "index.php", false);
			exit;
	...
	case "permission_group":
		...
		if(!$config['permission']) $func->transfer("Trang không tồn tại", "index.php", false);
		if($func->check_permission())
		{
			$func->transfer("Bạn không có quyền vào trang này", "index.php", false);
			exit;
...
function save_item_admin()
{
	...
	else
	{
		...
		if($data['password']=="") $func->transfer("Chưa nhập mật khẩu", "index.php?com=user&act=add_admin&p=".$curPage, false);
...
function save_item()
{
	...
	else
	{
		...
		if($data['password'] == "") $func->transfer("Chưa nhập mật khẩu", "index.php?com=user&act=add&p=".$curPage, false);
...
function edit()
{
	global $d, $func, $curPage, $item, $config;
	...
	if(!empty($_POST))
	{
		...
		if($changepass)
		{
			...
			if($old_pass!='' || $new_pass!='' ||  $renew_pass!='')
			{
				...
				/* Lấy dữ liệu */
				$row = $d->rawQueryOne("select id, password from #_user where username = ?",array($_SESSION['login_admin']['username']));
		...
		{
			$username = $data['username'];
			$row = $d->rawQueryOne("select id from #_user where username <> ? and username = ?",array($_SESSION['login_admin']['username'],$username));
	...
	$item = $d->rawQueryOne("select * from #_user where username = ?",array($_SESSION['login_admin']['username']));
}
