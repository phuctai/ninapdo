...
function save_item()
{
	...
	if($id)
	{
		...
		$d->where('id', $id);
		$d->where('role', 1);
		if($d->update('user',$data)) $func->transfer("Cập nhật dữ liệu thành công", "index.php?com=user&act=man&p=".$curPage);
	...
	else
	{
		...
		if($d->insert('user',$data)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=user&act=man&p=".$curPage);
