...
if(isset($_POST["listemail"]))
{
	...
	if($file = $func->uploadImage("file-taptin", $config['newsletter'][$type]['file_type'], UPLOAD_FILE,$file_name))
	{
		$data['taptin'] = $file;
	}
	...
	{
		$func->delete_file(UPLOAD_FILE.$file);
		$func->transfer("Email đã được gửi thành công.", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage);
	}
	else
	{
		$func->delete_file(UPLOAD_FILE.$file);
		$func->transfer("Email gửi bị lỗi. Vui lòng thử lại sau", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
	}
}
...
function get_item()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
	
	$item = $d->rawQueryOne("select * from #_newsletter where id = ? and type = ?",array($id,$type));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
}
...
function save_item()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
	...
	if($id)
	{
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
	}
	else
	{
		if($taptin = $func->uploadImage("file-taptin", $config['newsletter'][$type]['file_type'],UPLOAD_FILE,$file_name))
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
	}
}
...
function delete_item()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man&type=".$type."&p=".$curPage,0);
}
?>