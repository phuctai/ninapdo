...
if(!$config['website']['debug-developer']) $func->transfer("Trang không tồn tại", "index.php",0);
...
function get_create()
{
	...
	foreach($config['website']['lang'] as $k => $v)
	{
		...
		$langfile = fopen(LIBRARIES."lang/lang".$k.".php", "w") or $func->transfer("Không thể tạo tập tin.","index.php?com=lang&act=man&p=".$curPage,0);
...
function get_item()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=lang&act=man&p=".$curPage,0);	

	$item = $d->rawQueryOne("select * from #_lang where id = ?",array($id));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=lang&act=man&p=".$curPage,0);	
}
...
function save_item()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=lang&act=man&p=".$curPage,0);
	...
	if($id)
	{
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=lang&act=man&p=".$curPage,0);
	}
	else
	{
		if($d->insert('lang',$data)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=lang&act=man&p=".$curPage);
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=lang&act=man&p=".$curPage,0);
	}
}
...
function delete_item()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=lang&act=man&p=".$curPage,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=lang&act=man&p=".$curPage,0);
}
?>