...
function get_photo()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
	...
	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
}
...
function save_photo()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config[$com][$type][$dfgallery][$val]['thumb_width_photo'], $config[$com][$type][$dfgallery][$val]['thumb_height_photo'], "../upload/".$com."/",$file_name,$config[$com][$type][$dfgallery][$val]['thumb_ratio_photo']);
			...
			if($row['id'])
			{
				...
				$func->delete_file("../upload/".$com."/".$row['thumb']);
			}
		}
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
	}
	else
	{
		...
		if($numberPhoto)
		{
			for($i=0;$i<count($dataMultiTemp);$i++)
			{
				...
				{
					$file_name = $func->upload_name($_FILES["file-taptin".$i]["name"]);
				...
				if($config[$com][$type][$dfgallery][$val]['images_photo'])
				{
					...
					if($photo = $func->uploadImage("file".$i, $config[$com][$type][$dfgallery][$val]['img_type_photo'], "../upload/".$com."/",$file_name.$i))
					{
						...
						$dataMulti['thumb'] = $func->createThumb($dataMulti['photo'], $config[$com][$type][$dfgallery][$val]['thumb_width_photo'], $config[$com][$type][$dfgallery][$val]['thumb_height_photo'], "../upload/".$com."/",$file_name.$i,$config[$com][$type][$dfgallery][$val]['thumb_ratio_photo']);

						if(!$d->insert('gallery',$dataMulti)) $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
					}
				}
				else
				{
		...
		{
			$func->transfer("Dữ liệu rỗng", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
		}
	}
}
...
function delete_photo()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "Dữ liệu không có thực", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=".$com."&act=man_photo&idc=".$idc."&type=".$type."&kind=".$kind."&val=".$val."&p=".$curPage,0);
}
?>