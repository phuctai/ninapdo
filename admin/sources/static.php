...
function save_static()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=static&act=capnhat&type=".$type,0);
	...
	{
		$data['photo'] = $photo;
		$data['thumb'] = $func->createThumb($data['photo'], $config['static'][$type]['thumb_width'], $config['static'][$type]['thumb_height'], UPLOAD_NEWS,$file_name,$config['static'][$type]['thumb_ratio']);
	...
	if($static['id'])
	{
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=static&act=capnhat&type=".$type,0);
	}
	else
	{
		if($data['tenvi']!='' || $data['motavi']!='' || $data['noidungvi']!='')
		{
			...
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=static&act=capnhat&type=".$type,0);
		}
		$func->transfer("Dữ liệu rỗng", "index.php?com=static&act=capnhat&type=".$type,0);
	}
}
?>