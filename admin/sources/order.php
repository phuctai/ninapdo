...
if(!$config['order']['active']) $func->transfer("Trang không tồn tại", "index.php",0);
...
function get_item()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=order&act=man&p=".$curPage,0);
	
	$item = $d->rawQueryOne("select * from #_order where id = ?",array($id));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=order&act=man&p=".$curPage,0);
...
function save_item()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=order&act=man&p=".$curPage,0);
	...
	if($id)
	{
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=order&act=man&p=".$curPage,0);
	}
	else
	{
		$func->transfer("Dữ liệu rỗng", "index.php?com=order&act=man&p=".$curPage,0);
	}
}
...
function delete_item()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=order&act=man&p=".$curPage,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=order&act=man&p=".$curPage,0);
}
?>