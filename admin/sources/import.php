...
function editImages()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=import&act=man&type=".$type."&p=".$curPage,0);

	$item = $d->rawQueryOne("select * from #_excel where id = ? and type = ?",array($id,$type));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=import&act=man&type=".$type."&p=".$curPage,0);
}
...
function saveImages()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=import&act=man&type=".$type."&p=".$curPage,0);
	...
	if($id)
	{
		...
		if($photo = $func->uploadImage("file", $config['import']['img_type'], UPLOAD_EXCEL, $file_name))
		{
			...
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=import&act=man&type=".$type."&p=".$curPage,0);
		}
		else
		{
			$func->transfer("Không nhận được hình ảnh mới", "index.php?com=import&act=editImages&id=".$id."&type=".$type."&p=".$curPage,0);
		}
	}
	else
	{
		$func->transfer("Không nhận được dữ liệu", "index.php?com=import&act=man&type=".$type."&p=".$curPage,0);
	}
}
...
function uploadImages()
{
	...
	if(isset($_POST['uploadImg']) && isset($_FILES['files']) && $_FILES['files']["name"][0]!='') 
	{
        ...
        for($i=0;$i<$fileCount;$i++) 
        {
        	if($_FILES['files']['name'][$i]!='')
			{
				...
				{
					$random = $func->digitalRandom(0,9,6);
    ...
    {
    	$func->transfer("Dữ liệu rỗng", "index.php?com=import&act=man&type=".$type,0);
    }
}
...
function deleteImages()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=import&act=man&type=".$type."&p=".$curPage,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=import&act=man&type=".$type."&p=".$curPage,0);
}
...
function uploadExcel()
{
	...
	if(isset($_POST['importExcel']))
	{
		...
		if($file_type == "application/vnd.ms-excel" || $file_type == "application/x-ms-excel")
		{
			...
			foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) 
			{
				...
				for($row=2;$row<=$highestRow;++$row)
				{
					...
					if($masp!="")
					{
						...
						$data['tenkhongdau'] = $func->changeTitle($data['tenvi']);
						$data['gia'] = $gia;
						$data['giamoi'] = $giamoi;
						$data['giakm'] = $giakm;
			...
			{
				$func->transfer($mess, "index.php?com=import&act=man&type=".$type,0);
			}
		}
		else
		{
