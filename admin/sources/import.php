				...
				foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) 
				{
					...
					for($row=2;$row<=$highestRow;++$row)
					{
						...
						if($masp!="")
						{
							...
							$idlist = $d->rawQueryOne("SELECT id FROM table_product_list WHERE tenkhongdauvi = ?",array($tkdcap1));

							/* Lấy danh mục cấp 2 */
							$tkdcap2 = $func->changeTitle($cap2);
							$idcat = $d->rawQueryOne("SELECT id FROM table_product_cat WHERE tenkhongdauvi = ?",array($tkdcap2));
