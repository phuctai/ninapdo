...
function get_item()
{
	global $d, $func, $strUrl, $curPage, $item, $attributes, $gallery, $type, $com;

	$id = htmlspecialchars($_GET['id']);

	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);

	$item = $d->rawQueryOne("select * from #_product where id = ? and type = ?",array($id,$type));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);
...
function save_item()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);
	...
	if($id && $act!='save_copy')
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['product'][$type]['thumb_width'], $config['product'][$type]['thumb_height'], UPLOAD_PRODUCT,$file_name,$config['product'][$type]['thumb_ratio']);
			...
			if($row['id'])
			{
				...
				$func->delete_file(UPLOAD_PRODUCT.$row['thumb']);
			}
		}

		$file_name = $func->upload_name($_FILES['file-taptin']["name"]);
		...
		if(isset($_FILES['files'])) 
		{
            ...
            $file_name = $func->digitalRandom(0,9,6);
            	...
            	if($_FILES['files']['name'][$i]!='')
				{
					if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
					{
						if(move_uploaded_file($myFile["tmp_name"][$i], UPLOAD_PRODUCT."/".$file_name."_".$myFile["name"][$i]))
			            {
							...
							$data1['thumb'] = $func->createThumb($data1['photo'], $config['product'][$type]['gallery'][$type]['thumb_width_photo'], $config['product'][$type]['gallery'][$type]['thumb_height_photo'], UPLOAD_PRODUCT, $file_name."_".$myFile["name"][$i],$config['product'][$type]['gallery'][$type]['thumb_ratio_photo']);	
		...
		{
			if($savehere) $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=edit&type=".$type."&p=".$curPage.$strUrl."&id=".$id,0);
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);
		}
	}
	else
	{
		...
		else
		{
			if($act=='save_copy')
			{
				if($savehere) $func->transfer("Sao chép dữ liệu bị lỗi", "index.php?com=product&act=edit&type=".$type."&p=".$curPage.$strUrl."&id=".$id_insert,0);
				else $func->transfer("Sao chép dữ liệu bị lỗi", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);
			}
			else
			{
				if($savehere) $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=edit&type=".$type."&p=".$curPage.$strUrl."&id=".$id_insert,0);
				else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);
			}
		}
	}
}
...
function delete_item()
{
	...
	if($id)
	{
		...
		if($row['id'])
		{
			...
			if(count($row))
			{
				foreach($row as $v)
				{
					...
					$func->delete_file(UPLOAD_PRODUCT.$v['thumb']);
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function get_item_size()
{
	...
	$id = htmlspecialchars($_GET['id']);
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_size&type=".$type."&p=".$curPage,0);

	$item = $d->rawQueryOne("select * from #_product_size where id = ?",array($id));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_size&type=".$type."&p=".$curPage,0);
}
...
function save_item_size()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_size&type=".$type."&p=".$curPage,0);
	...
	if($id)
	{
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_size&type=".$type."&p=".$curPage,0);
	}
	else
	{
		$data['ngaytao'] = time();
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_size&type=".$type."&p=".$curPage,0);
	}
}
...
function delete_item_size()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_size&type=".$type."&p=".$curPage,0);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_size&type=".$type."&p=".$curPage,0);
}
...
function get_item_mau()
{
	...
	$id = htmlspecialchars($_GET['id']);
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_mau&type=".$type."&p=".$curPage,0);

	$item = $d->rawQueryOne("select * from #_product_mau where id = ?",array($id));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_mau&type=".$type."&p=".$curPage,0);
}
...
function save_item_mau()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_mau&type=".$type."&p=".$curPage,0);
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['product'][$type]['thumb_width_mau'], $config['product'][$type]['thumb_height_mau'], UPLOAD_COLOR,$file_name,$config['product'][$type]['thumb_ratio_mau']);
			...
			if($row['id'])
			{
				...
				$func->delete_file(UPLOAD_COLOR.$row['thumb']);
			}
		}
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_mau&type=".$type."&p=".$curPage,0);
	}
	else
	{	
		if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_mau'], UPLOAD_COLOR,$file_name))
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_mau&type=".$type."&p=".$curPage,0);
	}
}
...
function delete_item_mau()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_mau&type=".$type."&p=".$curPage,0);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_mau&type=".$type."&p=".$curPage,0);
}
...
function get_list()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list&type=".$type."&p=".$curPage.$strUrl,0);

	$item = $d->rawQueryOne("select * from #_product_list where id = ? and type = ?",array($id,$type));
	
	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_list&type=".$type."&p=".$curPage.$strUrl,0);
...
function save_list()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list&type=".$type."&p=".$curPage.$strUrl,0);
	...
	{					
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['product'][$type]['thumb_width_list'], $config['product'][$type]['thumb_height_list'], UPLOAD_PRODUCT,$file_name,$config['product'][$type]['thumb_ratio_list']);
            ...
            for($i=0;$i<$fileCount;$i++) 
            {
            	if($_FILES['files']['name'][$i]!='')
            	{
					if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
					{
			            ...
			            {
							$data1['photo'] = $file_name."_".$myFile["name"][$i];
							$data1['thumb'] = $func->createThumb($data1['photo'], $config['product'][$type]['gallery_list'][$type]['thumb_width_photo'], $config['product'][$type]['gallery_list'][$type]['thumb_height_photo'], UPLOAD_PRODUCT, $file_name."_".$myFile["name"][$i],$config['product'][$type]['gallery_list'][$type]['thumb_ratio_photo']);	
							$data1['stt'] = (int)$_POST['stt-filer'][$dem];		
							$data1['tenvi'] = $_POST['ten-filer'][$dem];
							$data1['id_photo'] = $id;
							$data1['com'] = $com;
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_list&type=".$type."&p=".$curPage.$strUrl,0);
	}
	else
	{				
		if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT,$file_name)){
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_list&type=".$type."&p=".$curPage.$strUrl,0);
	}
}
...
function delete_list()
{
	...
	if(isset($_GET['id']))
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_list&type=".$type."&p=".$curPage.$strUrl,0);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function get_cat()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_cat&type=".$type."&p=".$curPage.$strUrl,0);
	
	$item = $d->rawQueryOne("select * from #_product_cat where id = ? and type = ?",array($id,$type));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_cat&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function save_cat()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_cat&type=".$type."&p=".$curPage.$strUrl,0);
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['product'][$type]['thumb_width_cat'], $config['product'][$type]['thumb_height_cat'], UPLOAD_PRODUCT,$file_name,$config['product'][$type]['thumb_ratio_cat']);			
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_cat&type=".$type."&p=".$curPage.$strUrl,0);
	}
	else
	{		
		if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_cat'], UPLOAD_PRODUCT,$file_name))
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_cat&type=".$type."&p=".$curPage.$strUrl,0);
	}
}
...
function delete_cat()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_cat&type=".$type."&p=".$curPage.$strUrl,0);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_cat&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function get_loai()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_item&type=".$type."&p=".$curPage.$strUrl,0);
	
	$item = $d->rawQueryOne("select * from #_product_item where id = ? and type = ?",array($id,$type));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_item&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function save_loai()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_item&type=".$type."&p=".$curPage.$strUrl,0);
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['product'][$type]['thumb_width_item'], $config['product'][$type]['thumb_height_item'], UPLOAD_PRODUCT,$file_name,$config['product'][$type]['thumb_ratio_item']);		
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_item&type=".$type."&p=".$curPage.$strUrl,0);
	}
	else
	{	
		if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_item'], UPLOAD_PRODUCT,$file_name))
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_item&type=".$type."&p=".$curPage.$strUrl,0);
	}
}
...
function delete_loai()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_item&type=".$type."&p=".$curPage.$strUrl,0);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_item&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function get_sub()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_sub&type=".$type."&p=".$curPage.$strUrl,0);

	$item = $d->rawQueryOne("select * from #_product_sub where id = ? and type = ?",array($id,$type));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_sub&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function save_sub()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_sub&type=".$type."&p=".$curPage.$strUrl,0);
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['product'][$type]['thumb_width_sub'], $config['product'][$type]['thumb_height_sub'], UPLOAD_PRODUCT,$file_name,$config['product'][$type]['thumb_ratio_sub']);			
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_sub&type=".$type."&p=".$curPage.$strUrl,0);
	}
	else
	{	
		if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_sub'], UPLOAD_PRODUCT,$file_name))
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_sub&type=".$type."&p=".$curPage.$strUrl,0);
	}
}
...
function delete_sub()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_sub&type=".$type."&p=".$curPage.$strUrl,0);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_sub&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function get_brand()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_brand&type=".$type."&p=".$curPage.$strUrl,0);

	$item = $d->rawQueryOne("select * from #_product_brand where id = ? and type = ?",array($id,$type));
	
	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_brand&type=".$type."&p=".$curPage.$strUrl,0);
}
...
function save_brand()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_brand&type=".$type."&p=".$curPage,0);
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['product'][$type]['thumb_width_brand'], $config['product'][$type]['thumb_height_brand'], UPLOAD_PRODUCT,$file_name,$config['product'][$type]['thumb_ratio_brand']);		
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_brand&type=".$type."&p=".$curPage,0);
	}
	else
	{				
		if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_brand'], UPLOAD_PRODUCT,$file_name))
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_brand&type=".$type."&p=".$curPage,0);
	}
}
...
function delete_brand()
{
	...
	if(isset($_GET['id']))
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_brand&type=".$type."&p=".$curPage.$strUrl,0);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_brand&type=".$type."&p=".$curPage.$strUrl,0);
}
?>