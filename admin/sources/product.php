<?php
	...
	function save_item()
	{
		...
		$data['giamoi'] = ($data['giamoi']) ? str_replace(",","",$data['giamoi']) : 0;
		$data['giakm'] = ($data['giakm']) ? $data['giakm'] : 0;
		...
		if($id && $act!='save_copy')
		{
			...
			{
				$data['taptin'] = $taptin;
				$row = $d->rawQueryOne("select id, taptin from #_product where id = ? and type = ? limit 0,1",array($id,$type));
	...
	function delete_item()
	{
		...
		if($id)
		{
			...
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo, taptin from #_product where id = ? and type = ? limit 0,1",array($id,$type));
