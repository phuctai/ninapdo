...
function save_item()
{
	...
	if($id && $act!='save_copy')
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['news'][$type]['thumb_width'], $config['news'][$type]['thumb_height'], UPLOAD_NEWS,$file_name,$config['news'][$type]['thumb_ratio']);
			...
			if($row['id'])
			{
				...
				$func->delete_file(UPLOAD_NEWS.$row['thumb']);
			}
		}
            	...
            	if($_FILES['files']['name'][$i]!='')
				{
					if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
					{
						if(move_uploaded_file($myFile["tmp_name"][$i], UPLOAD_NEWS."/".$file_name."_".$myFile["name"][$i]))
			            {
							...
							$data1['thumb'] = $func->createThumb($data1['photo'], $config['news'][$type]['gallery'][$type]['thumb_width_photo'], $config['news'][$type]['gallery'][$type]['thumb_height_photo'], UPLOAD_NEWS, $file_name."_".$myFile["name"][$i],$config['news'][$type]['gallery'][$type]['thumb_ratio_photo']);	
		...
		if($row['id'])
		{
			...
			if(count($row))
			{
				foreach($row as $v)
				{
					...
					$func->delete_file(UPLOAD_NEWS.$v['thumb']);
...
function save_list()
{
	...
	{					
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['news'][$type]['thumb_width_list'], $config['news'][$type]['thumb_height_list'], UPLOAD_NEWS,$file_name,$config['news'][$type]['thumb_ratio_list']);
            ...
            for($i=0;$i<$fileCount;$i++) 
            {
            	if($_FILES['files']['name'][$i]!='')
            	{
					if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
					{
			            ...
			            {
							$data1['photo'] = $file_name."_".$myFile["name"][$i];
							$data1['thumb'] = $func->createThumb($data1['photo'], $config['news'][$type]['gallery_list'][$type]['thumb_width_photo'], $config['news'][$type]['gallery_list'][$type]['thumb_height_photo'], UPLOAD_NEWS, $file_name."_".$myFile["name"][$i],$config['news'][$type]['gallery_list'][$type]['thumb_ratio_photo']);	
							$data1['stt'] = (int)$_POST['stt-filer'][$dem];		
							$data1['tenvi'] = $_POST['ten-filer'][$dem];
							$data1['id_photo'] = $id;
							$data1['com'] = $com;
...
function save_cat()
{
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['news'][$type]['thumb_width_cat'], $config['news'][$type]['thumb_height_cat'], UPLOAD_NEWS,$file_name,$config['news'][$type]['thumb_ratio_cat']);			
...
function save_loai()
{
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['news'][$type]['thumb_width_item'], $config['news'][$type]['thumb_height_item'], UPLOAD_NEWS,$file_name,$config['news'][$type]['thumb_ratio_item']);		
...
function save_sub()
{
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['news'][$type]['thumb_width_sub'], $config['news'][$type]['thumb_height_sub'], UPLOAD_NEWS,$file_name,$config['news'][$type]['thumb_ratio_sub']);			
