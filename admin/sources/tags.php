...
function get_item()
{
	...
	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=tags&act=man&type=".$type."&p=".$curPage,0);

	$item = $d->rawQueryOne("select * from #_tags where id = ? and type = ?",array($id,$type));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=tags&act=man&type=".$type."&p=".$curPage,0);
}
...
function save_item()
{
	...
	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=tags&act=man&type=".$type."&p=".$curPage,0);
	...
	if($id)
	{
		...
		{
			$data['photo'] = $photo;
			$data['thumb'] = $func->createThumb($data['photo'], $config['tags'][$type]['thumb_width'], $config['tags'][$type]['thumb_height'], UPLOAD_TAGS,$file_name,$config['tags'][$type]['thumb_ratio']);			
			...
			if($row['id'])
			{
				...
				$func->delete_file(UPLOAD_TAGS.$row['thumb']);
			}
		}
		...
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=tags&act=man&type=".$type."&p=".$curPage,0);
	}
	else
	{
		if($photo = $func->uploadImage("file", $config['tags'][$type]['img_type'], UPLOAD_TAGS,$file_name))
		...
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=tags&act=man&type=".$type."&p=".$curPage,0);
	}
}
...
function delete_item()
{
	...
	if($id)
	{
		...
		else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=tags&act=man&type=".$type."&p=".$curPage,0);
	}
	...
	else $func->transfer("Không nhận được dữ liệu", "index.php?com=tags&act=man&type=".$type."&p=".$curPage,0);
}
?>