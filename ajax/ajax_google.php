<?php
	...
	$user_gg = $d->rawQueryOne("SELECT id FROM table_user WHERE email = ?",array($gg_email));

	if($user_gg['id'])
	{
		$row_user = $d->rawQueryOne("SELECT * FROM table_user WHERE hienthi>0 AND email = ?",array($gg_email));
	...
	else
	{
		...
		$data['email']=$gg_email;
		$data['avatar']=$func->uploadUrl($gg_img,UPLOAD_USER,"jpg,bmp,jpeg,png,gif","20000000");
		$data['hienthi']=1;
		$data['id_social']=1;
		$d->insert('user',$data);
