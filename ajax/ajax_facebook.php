<?php
	...
	$user_fb = $d->rawQueryOne("SELECT id FROM table_user WHERE email = ?",array($fb_email));

	if($user_fb['id']!='')
	{
		$row_user = $d->rawQueryOne("SELECT * FROM table_user WHERE hienthi>0 AND email = ?",array($fb_email));
