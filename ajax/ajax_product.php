<?php 
	...
	/* Get data */
	$sql = "SELECT ten$lang, tenkhongdauvi, tenkhongdauen, id, photo, gia, giamoi, giakm, type FROM table_product WHERE hienthi=1 AND noibat>0 AND type='san-pham' ORDER BY stt,id DESC";
		...
		<?php for($i=0;$i<count($items);$i++) { ?>
			<div class="product">
				<a class="box-product text-decoration-none" href="<?=$items[$i][$sluglang]?>" title="<?=$items[$i]['ten'.$lang]?>">
					<p class="pic-product scale-img"><img onerror="this.src='//placehold.it/270x270';" src="<?=WATERMARK?>/product/270x270x1/<?=UPLOAD_PRODUCT_L.$items[$i]['photo']?>" alt="<?=$items[$i]['ten'.$lang]?>"/></p>
