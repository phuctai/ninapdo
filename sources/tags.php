<?php  
	...
	{
		/* Lấy tag detail */
		$tags_detail = $d->rawQueryOne("select id, ten$lang, type, photo, tenkhongdauvi, tenkhongdauen from #_tags where id = ? and type = ?",array($id,$type));
		...
		$img_json_bar = json_decode($tags_detail['options'],true);
		...
		/* breadCrumbs */
		if($title_crumb) $breadcr->setBreadCrumbs($tags_detail[$sluglang],$title_crumb);
