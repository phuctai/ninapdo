<?php
	...
	if(isset($_POST['thanhtoan']))
	{
	    ...
	    $diachi = htmlspecialchars($_POST['diachi']).', '.get_places("wards",$wards).', '.get_places("district",$district).', '.get_places("city",$city);
	    $httt = htmlspecialchars($_POST['payments']);
	    $htttText = get_payments($httt);
											...
											<tbody>
												<tr>
													<td>
														...
														<p style="margin:4px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal">Quý khách nhận được thông tin đặt hàng tại website '.$websitecongty.'</p>
														<h3 style="font-size:13px;font-weight:bold;color:'.$mauform.';text-transform:uppercase;margin:20px 0 0 0;padding: 0 0 5px;border-bottom:1px solid #ddd">Thông tin đơn hàng #'.$madonhang.' <span style="font-size:12px;color:#777;text-transform:none;font-weight:normal">(Ngày '.date('d',$ngaygui).' tháng '.date('m',$ngaygui).' năm '.date('Y H:i:s',$ngaygui).')</span></h3>
													</td>
												</tr>
												<tr>
													...
													<table border="0" cellpadding="0" cellspacing="0" width="100%">
														...
														<tbody>
															...
															<tr>
																...
																if($ship)
																{
																	$noidung.='<br><strong>Phí vận chuyển: </strong> '.number_format($ship,0,'','.').'đ';
																}
																if($endowID)
																{
																	if($endowType==1)
																	{
																		$noidung.='<br><strong>Ưu đãi: </strong> -'.$endow.'%';
																	}
																	else if($endowType==2)
																	{
																		$noidung.='<br><strong>Ưu đãi: </strong> -'.$endow.'đ';
																	}
																}
																$noidung.='</td>
															</tr>
												...
												<tr>
													<td>
													<h2 style="text-align:left;margin:10px 0;border-bottom:1px solid #ddd;padding-bottom:5px;font-size:13px;color:'.$mauform.'">CHI TIẾT ĐƠN HÀNG</h2>
													<table border="0" cellpadding="0" cellspacing="0" style="background:#f5f5f5" width="100%">
														<thead>
															<tr>
																<th align="left" bgcolor="'.$mauform.'" style="padding:6px 9px;color:#fff;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:14px">Sản phẩm</th>
																<th align="left" bgcolor="'.$mauform.'" style="padding:6px 9px;color:#fff;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:14px">Đơn giá</th>
																<th align="center" bgcolor="'.$mauform.'" style="padding:6px 9px;color:#fff;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:14px;min-width:55px;">Số lượng</th>
																<th align="right" bgcolor="'.$mauform.'" style="padding:6px 9px;color:#fff;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:14px">Tổng tạm</th>
															</tr>
														</thead>
														'.$chitietdonhang.'
													</table>
												...
												<tr>
													<td>
													<p style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal;margin-top:10px;text-align:right"><strong><a href="'.$trangchu.'" style="color:'.$mauform.';text-decoration:none;font-size:14px" target="_blank">'.$tencongty.'</a> </strong></p>
													</td>
												</tr>
											</tbody>
										</table>
				...
				<tr>
					...
					<table width="600">
						<tbody>
							<tr>
								<td>
								<p align="left" style="font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:18px;color:#4b8da5;padding:10px 0;margin:0px;font-weight:normal">Quý khách nhận được email này vì đã mua hàng tại '.$websitecongty.'.<br>
								Để chắc chắn luôn nhận được email thông báo, xác nhận mua hàng từ '.$websitecongty.', quý khách vui lòng thêm địa chỉ <strong><a href="mailto:'.$emailhosting.'" target="_blank">'.$emailhosting.'</a></strong> vào số địa chỉ (Address Book, Contacts) của hộp email.<br>
									...
									<td align="left" height="auto" style="padding:15px" width="600">
										<table style="width:100%;">
											<tbody>
												<tr>
													<td>
														<h1 style="font-size:17px;font-weight:bold;color:#444;padding:0 0 5px 0;margin:0">Cảm ơn quý khách đã đặt hàng tại '.$websitecongty.'</h1>
														<p style="margin:4px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal">Chúng tôi rất vui thông báo đơn hàng #'.$madonhang.' của quý khách đã được tiếp nhận và đang trong quá trình xử lý. '.$websitecongty.' sẽ thông báo đến quý khách ngay khi hàng chuẩn bị được giao.</p>
														<h3 style="font-size:13px;font-weight:bold;color:'.$mauform.';text-transform:uppercase;margin:20px 0 0 0;padding: 0 0 5px;border-bottom:1px solid #ddd">Thông tin đơn hàng #'.$madonhang.' <span style="font-size:12px;color:#777;text-transform:none;font-weight:normal">(Ngày '.date('d',$ngaygui).' tháng '.date('m',$ngaygui).' năm '.date('Y H:i:s',$ngaygui).')</span></h3>
													</td>
												</tr>
											<tr>
											...
											<table border="0" cellpadding="0" cellspacing="0" width="100%">
												...
												<tbody>
													...
													<tr>
														...
														if($ship)
														{
															$noidungkh.='<br><strong>Phí vận chuyển: </strong> '.number_format($ship,0,'','.').'đ';
														}
														if($endowID)
														{
															if($endowType==1)
															{
																$noidungkh.='<br><strong>Ưu đãi: </strong> -'.$endow.'%';
															}
															else if($endowType==2)
															{
																$noidungkh.='<br><strong>Ưu đãi: </strong> -'.$endow.'đ';
															}
														}
														$noidungkh.='</td>
													</tr>
										...
										<tr>
											...
											<div style="margin:auto;text-align:center"><a href="'.$linkchitiet.'" style="display:inline-block;text-decoration:none;background-color:'.$mauform.'!important;text-align:center;border-radius:3px;color:#fff;padding:5px 10px;font-size:12px;font-weight:bold;margin-top:5px" target="_blank">Chi tiết đơn hàng tại '.$websitecongty.'</a></div>
											</td>
										</tr>
										<tr>
											<td>&nbsp;
		...
		/* lưu đơn hàng */
		$data_donhang['id_user'] = $_SESSION[$login_name]['id'];
		...
		if($id_insert)
		{
			for($i=0;$i<$max;$i++)
			{
				...
				$proinfo = get_product_info($pid);
				$gia = get_price($pid);
				$giamoi = get_price_new($pid);
				$color = get_product_mau($_SESSION['cart'][$i]['mau']);
		...
		$message = $noidung;
		sendEmail("admin", $arrayEmail, $subject, $message, $file);
		...
		$subject = "Thông tin đơn hàng từ ".$setting['ten'.$lang];
		$message = $noidungkh;
		sendEmail("customer", $arrayEmail, $subject, $message, $file);
	    ...
	    /* Xóa giỏ hàng */
        unset($_SESSION['cart']);
		transfer("Thông tin đơn hàng đã được gửi thành công.", $config_base);
	}
?>