<?php
	...
	$kind = htmlspecialchars($match['params']['kind']);
	...
	switch($kind)
	{
		case 'dang-nhap':
			...
			if(isset($_SESSION[$login_name][$login_name])){$func->transfer("Trang không tồn tại",$config_base,0);}
	...
	function info_user()
	{
		global $d, $func, $row_detail, $config_base, $login_name;

		$iduser = $_SESSION[$login_name]['id'];

		if($iduser)
		{
			$row_detail = $d->rawQueryOne("select ten, username, gioitinh, ngaysinh, email, dienthoai, diachi from #_member where id = ?",array($iduser));
		    ...
		    if(isset($_POST['capnhatthongtin']))
		    {
		        ...
		        if($password)
		        {
		            $row = $d->rawQueryOne("select id from #_member where id = ? and password = ?",array($iduser,$passwordMD5));

		            if(!$row['id']) $func->transfer("Mật khẩu cũ không chính xác","",0);
		            if(!$new_password || ($new_password != $new_password_confirm)) $func->transfer("Thông tin mật khẩu mới không chính xác","",0);
		...
		{
			$func->transfer("Trang không tồn tại",$config_base,0);
		}
	}
	...
	function active_user()
	{
		...
		/* Kiểm tra thông tin */
        $row_detail = $d->rawQueryOne("select hienthi, maxacnhan, id from #_member where id = ?",array($id));

        if(!$row_detail['id']) $func->transfer("Tài khoản của bạn chưa được kích hoạt",$config_base,0);
        ...
        else
        {
        	...
        	{
        		$func->transfer("Mã xác nhận không đúng. Vui lòng nhập lại mã xác nhận.",$config_base."account/kich-hoat?id=".$id,0);
        	}
        }
	}
	...
	function login()
	{
		global $d, $func, $login_name, $config_base;
		...
		if(!$username) $func->transfer("Chưa nhập tên tài khoản",'account/dang-nhap',0);
		if(!$password) $func->transfer("Chưa nhập mật khẩu",'account/dang-nhap',0);
		
		$row = $d->rawQueryOne("select id, password, username, dienthoai, diachi, email, ten from #_member where username = ? and hienthi = 1",array($username));
		...
		if($row['id'])
		{
			if($row['password'] == $passwordMD5)
			{
				...
				$_SESSION[$login_name]['login_session'] = $login_session;
				...
				if($remember)
				{
					...
					setcookie('login_id',$row['id'],$time_expiry,'/');
			...
			{
				$func->transfer("Tên đăng nhập hoặc mật khẩu không chính xác. Hoặc tài khoản của bạn chưa được xác nhận từ Quản trị website", $config_base."account/dang-nhap",0);
			}
		}
		else
		{
	...
	function signup()
	{
		...
		if($password != $repassword) $func->transfer("Xác nhận mật khẩu không trùng khớp", $config_base."account/dang-ky",0);

		/* Kiểm tra tên đăng ký */
		$row = $d->rawQueryOne("select id from #_member where username = ?",array($username));
		if($row['id']) $func->transfer("Tên đăng nhập đã tồn tại", $config_base."account/dang-ky",0);

		/* Kiểm tra email đăng ký */
		$row = $d->rawQueryOne("select id from #_member where email = ?",array($email));
		if($row['id']) $func->transfer("Địa chỉ email đã tồn tại", $config_base."account/dang-ky",0);
		...
		{
			$func->transfer("Đăng ký thành viên thất bại. Vui lòng thử lại sau.", $config_base,0);
		}
	}

	function send_active_user($username)
	{
		global $d, $setting, $func, $config_base, $lang;

		/* Lấy thông tin người dùng */
		$row = $d->rawQueryOne("select id, maxacnhan, username, password, ten, email, dienthoai, diachi from #_member where username = ?",array($username));
		...
		if(!$func->sendEmail("customer", $arrayEmail, $subject, $message, $file)) $func->transfer("Có lỗi xảy ra trong quá trình kích hoạt tài khoản. Vui lòng liên hệ với chúng tôi.",$config_base."lien-he",0);
	}

	function doimatkhau_user()
	{
		global $d, $setting, $func, $login_name, $config_base, $lang;
		...
		if(!$username) $func->transfer("Chưa nhập tên tài khoản", $config_base."account/quen-mat-khau",0);
		if(!$email) $func->transfer("Chưa nhập email đăng ký tài khoản", $config_base."account/quen-mat-khau",0);

		/* Kiểm tra username và email */
		$row = $d->rawQueryOne("select id from #_member where username = ? and email = ?",array($username,$email));
		if(!$row['id']) $func->transfer("Tên đăng nhập và email không tồn tại", $config_base."account/quen-mat-khau",0);
		...
		/* Lấy thông tin người dùng */
		$row = $d->rawQueryOne("select id, username, password, ten, email, dienthoai, diachi from #_member where username = ?",array($username));
		...
		if($func->sendEmail("customer", $arrayEmail, $subject, $message, $file))
		{
			$_SESSION[$login_name][$login_name] = false;
			unset($_SESSION[$login_name]);
			setcookie('login_id',"",-1,'/');
		...
		{
			$func->transfer("Có lỗi xảy ra trong quá trình cấp lại mật khẩu. Vui lòng liện hệ với chúng tôi.", $config_base."account/quen-mat-khau",0);
		}
	}
