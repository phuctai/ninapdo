<?php 
	...
	if(isset($_POST['submit-contact']))
	{
		...
		if($scopeRecaptcha >= 0.5)
		{
			...
			if($func->sendEmail("admin", $arrayEmail, $subject, $message, $file))
			{
				/* Send email customer */
				$arrayEmail = array(
					"dataEmail" => array(
				...
				if($func->sendEmail("customer", $arrayEmail, $subject, $message, $file)) $func->transfer("Gửi liên hệ thành công",$config_base);
			}
			else $func->transfer("Gửi liên hệ thất bại. Vui lòng thử lại sau",$config_base,0);
		}
		else
		{
			$func->transfer("Gửi liên hệ thất bại. Vui lòng thử lại sau",$config_base,0);
		}
	}
	...
	/* breadCrumbs */
	if($title_crumb) $data['breadcrumbs'][] = array('slug'=>$func->get_comlang('lien-he',$lang),'name'=>$title_crumb);
