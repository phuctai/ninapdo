	...
	class functions
	{
		...
		public function miniCssSet($link_css, $link=false)
		{
		    ...
		    else
		    {
		        ...
		        if(filesize($link_css)!=0) return $this->compress(fread($myfile,filesize($link_css)));
		    }
		}

		/* Upload name */
		public function upload_name($name)
		{
			$rand = rand(1000,9999);
		...
		/* Check slug lang */
		public function check_sluglang($lang)
		{
		...
		/* Lấy đường dẫn */
		public function get_slug($lang='vi',$id,$table)
		{
			global $d, $config;

			if($config['website']['slug']['lang-active'])
			{
				if($this->check_sluglang($lang)) $langTemp = $lang;
				else $langTemp = "vi";
			}
			else $langTemp = "vi";

			$row = $this->d->rawQueryOne("select tenkhongdau$langTemp from #_$table where id = ?",array($id));
		...
		public function checkLogin()
		{
			...
			if($iduser)
			{
				$row = $this->d->rawQueryOne("select * from #_user where id = ? and hienthi = 1 and role = 1",array($iduser));
			    ...
			    else
			    {
					...
					$this->transfer("Tài khoản của bạn đã hêt hạn truy cập. Vui lòng truy cập lại", $config_base."account/dang-nhap",0);
			    }
			}
		}
		...
		/* Create thumb */
		public function createThumb($file, $width, $height, $folder,$file_name,$zoom_crop='1')
		{
			$new_width   = $width;
			$new_height   = $height;
