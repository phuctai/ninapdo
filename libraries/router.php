<?php
    ...
    $router->map('GET', 'watermark/[i:w]x[i:h]x[i:z]/[**:src]', function($w,$h,$z,$src){
        global $func, $d;
        $watermark = $d->rawQueryOne("SELECT hienthi, photo, posleft, postop FROM table_photo WHERE type = ? AND act = ?",array('watermark','photo_static'));
        $func->createThumb($w,$h,$z,$src,$watermark);
    },'watermark');
    $router->map('GET', 'watermark-news/[i:w]x[i:h]x[i:z]/[**:src]', function($w,$h,$z,$src){
        global $func, $d;
        $watermark = $d->rawQueryOne("SELECT hienthi, photo, posleft, postop FROM table_photo WHERE type = ? AND act = ?",array('watermark-news','photo_static'));
        $func->createThumb($w,$h,$z,$src,$watermark);
    },'watermarkNews');
    $match = $router->match();
	if(is_array($match))
