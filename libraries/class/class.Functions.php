<?php
	class Functions
	{
		...
		public function check_login()
		{
			...
			$row = $this->d->rawQuery("select * from #_user where quyen = ? and hienthi>0",array($token));
		...
		public function createThumb($width_thumb, $height_thumb, $zoom_crop='1', $src, $watermark=null, $path=THUMBS, $preview=false, $args=array())
		{
			...
			{
				$upload_dir = THUMBS.'/'.WATERMARK.'/'.$path.'/'.$width_thumb.'x'.$height_thumb.'x'.$zoom_crop.'/';
			}
			else
			{
				if($watermark) $upload_dir = THUMBS.'/'.WATERMARK.'/'.$path.'/'.$width_thumb.'x'.$height_thumb.'x'.$zoom_crop.'/';
			...
			{
				$upload_dir = '';
				rmdir(THUMBS.'/'.WATERMARK.'/'.$path.'/'.$width_thumb.'x'.$height_thumb.'x'.$zoom_crop.'/');
				rmdir(THUMBS.'/'.WATERMARK.'/'.$path.'/');
			}
