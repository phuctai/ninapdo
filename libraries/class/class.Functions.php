<?php
	class Functions
	{
		...
		/* Create sitemap */
		public function createSitemap($type='',$table='',$level='',$time='',$changefreq='',$priority='',$lang,$orderby='')
		{
			global $config_base;
		...
		/* Transfer */
		public function transfer($msg, $page='', $stt=1)
		{
			global $config_base;
		...
		/* Create thumb */
		public function createThumb($width_thumb, $height_thumb, $zoom_crop='1', $src, $watermark=null)
		{
			$t = 3600*24*3;
			...
			if($watermark['hienthi']) $upload_dir = THUMBS.'/'.WATERMARK.'/'.$width_thumb.'x'.$height_thumb.'x'.$zoom_crop.'/'.$folder_old;	
			else $upload_dir = THUMBS.'/'.$width_thumb.'x'.$height_thumb.'x'.$zoom_crop.'/'.$folder_old;
			...
			{
				$overlay_gd_image = imagecreatefrompng(UPLOAD_PHOTO_L.$watermark['photo']);
				...
				switch($watermark['position'])
				{
					...
					case 2:
						$khoancachx = abs($new_width - $overlay_width)/2;
						$khoancachy = $overlay_padding;
						break;

					case 3:
						$khoancachx = abs($new_width - $overlay_width) - $overlay_padding;
						$khoancachy = $overlay_padding;
						break;

					case 4:
						$khoancachx = abs($new_width - $overlay_width) - $overlay_padding;
						$khoancachy = abs($new_height - $overlay_height)/2;
						break;

					case 5:
						$khoancachx = abs($new_width - $overlay_width) - $overlay_padding;
						$khoancachy = abs($new_height - $overlay_height) - $overlay_padding;
						break;
