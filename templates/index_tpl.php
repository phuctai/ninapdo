		...
		<div class="wrap-content d-flex align-items-center justify-content-between">
			...
			<div class="owl-carousel owl-theme owl-brand">
				<?php foreach($brand as $v) { ?>
					<div>
						<a class="brand text-decoration-none" href="<?=$func->get_slug($lang,$v['id'],'product_brand')?>" title="<?=$v['ten'.$lang]?>">
							<img onerror="this.src='//placehold.it/150x150';" src="<?=UPLOAD_PRODUCT_L?>150x150x2/<?=$v['photo']?>" alt="<?=$v['ten'.$lang]?>"/>
						</a>
					</div>
				<?php } ?>
			</div>
	...
	<div class="wrap-intro wrap-content d-flex align-items-start justify-content-between">
		<div class="left-intro">
			...
			<div class="newshome-intro w-clear">
				<a class="newshome-best text-decoration-none" href="<?=$func->get_slug($lang,$newsnb[0]['id'],'news')?>" title="<?=$newsnb[0]['ten'.$lang]?>">
					<p class="pic-newshome-best scale-img"><img onerror="this.src='//placehold.it/360x200';" src="<?=UPLOAD_NEWS_L?>360x200x1/<?=$newsnb[0]['photo']?>" alt="<?=$newsnb[0]['ten'.$lang]?>"></p>
				...
				<div class="newshome-scroll">
					<ul>
						<?php foreach($newsnb as $v) { ?>
							<li>
								<a class="newshome-normal text-decoration-none w-clear" href="<?=$func->get_slug($lang,$v['id'],'news')?>" title="<?=$v['ten'.$lang]?>">
									<p class="pic-newshome-normal scale-img"><img onerror="this.src='//placehold.it/150x120';" src="<?=UPLOAD_NEWS_L?>150x120x1/<?=$v['photo']?>" alt="<?=$v['ten'.$lang]?>"></p>
		...
		<div class="right-intro">
			...
			<div class="videohome-intro">
				<?=$func->getAddonsOnline("","videoHomeFotorama","videoFotorama",0,350,120,95)?>
	            <?php //$func->getAddonsOnline("","videoHomeSelect","videoSelect",0,0)?>
			</div>
		</div>
	</div>
<?php } ?>
				...
				<?php foreach($partner as $v) { ?>
					<div>
						<a class="partner text-decoration-none" href="<?=$v['link']?>" target="_blank" title="<?=$v['ten'.$lang]?>">
							<img onerror="this.src='//placehold.it/175x95';" src="<?=UPLOAD_PHOTO_L?>175x95x2/<?=$v['photo']?>" alt="<?=$v['ten'.$lang]?>"/>
						</a>
					</div>
				<?php } ?>
			</div>
