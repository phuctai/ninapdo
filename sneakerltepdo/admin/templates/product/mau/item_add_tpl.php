    ...
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
				<?php if($config['product'][$type]['mau_images']) { ?>
                    <div class="form-group">
                        <label class="change-photo" for="file">
                            <p>Upload hình ảnh:</p>
                            <div class="rounded">
                                <img class="rounded img-upload" src="<?=_upload_mau.$item['photo']?>" onerror="src='assets/images/noimage.png'" alt="Alt Photo"/>
                                <strong>
				...
				<?php } ?>
                <div class="row">
    				<?php if($config['product'][$type]['mau_mau']) { ?>
    				...
    				<?php } ?>
                	<?php if($config['product'][$type]['mau_gia']) { ?>
    			    	<div class="form-group col-md-3 col-sm-4">
    						<label class="d-block" for="ten">Giá:</label>
                            <div class="input-group">
                                <input type="text" class="form-control format-price" name="data[gia]" id="gia" placeholder="Giá bán" value="<?=$item['gia']?>">
    			    ...
    			    <?php } ?>
    				<?php if($config['product'][$type]['mau_loai'] && $config['product'][$type]['mau_images']) { ?>
    					<div class="form-group col-md-3 col-sm-4">
                            ...
                            <select class="form-control" name="data[loaihienthi]" id="loaihienthi">
                                ...
                                <option <?=($item['loaihienthi']==1)?"selected":""?> value="1">Hình ảnh</option>
                            </select>
                        </div>
    				<?php } ?>
                </div>
