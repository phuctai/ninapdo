...
<section class="content">
    <form method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <?php if($config[$com][$type][$dfgallery][$val]['cart_photo']) { ?>
        	<?php
        		$mau = $d->rawQueryOne("SELECT id_mau FROM table_product WHERE id = ?",array($idc));
        		if($mau['id_mau']!='')
        		{
					$idMau = explode(",",$mau['id_mau']);
					$cols = ["tenvi","id","mau","thumb","loaihienthi"];
					$d->where('id', $idMau, 'IN');
					$d->where('type', $type);
	    ...
	    <?php } ?>
		<?php $numberPhoto = $config[$com][$type][$dfgallery][$val]['number_photo']; ?>
		<?php for($i=0;$i<$numberPhoto;$i++) { $stt = $i+1; ?>
			<div class="card card-primary card-outline text-sm">
	            ...
	            <div class="card-body">
					<?php if($config[$com][$type][$dfgallery][$val]['images_photo']) { ?>
	                ...
	                <?php } ?>
	                <?php if($config[$com][$type][$dfgallery][$val]['mausac_photo']) { ?>
		            ...
		            <?php if($config[$com][$type][$dfgallery][$val]['tieude_photo'] || $config[$com][$type][$dfgallery][$val]['mota_photo'] || $config[$com][$type][$dfgallery][$val]['noidung_photo']) { ?>
		                        ...
		                        <div class="tab-content" id="custom-tabs-three-tabContent-lang">
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>-<?=$i?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    <?php if($config[$com][$type][$dfgallery][$val]['tieude_photo']) { ?>
		                                        <div class="form-group">
		                                    ...
		                                    <?php } ?>
		                                    <?php if($config[$com][$type][$dfgallery][$val]['mota_photo']) { ?>
		                                        <div class="form-group">
		                                            <label for="mota<?=$k?><?=$i?>">Mô tả (<?=$k?>):</label>
		                                            <textarea class="form-control <?=($config[$com][$type][$dfgallery][$val]['mota_cke_photo'])?'form-control-ckeditor':''?>" name="dataMulti[<?=$i?>][mota<?=$k?>]" id="mota<?=$k?><?=$i?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode($item['mota'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if($config[$com][$type][$dfgallery][$val]['noidung_photo']) { ?>
		                                        <div class="form-group">
		                                            <label for="noidung<?=$k?><?=$i?>">Nội dung (<?=$k?>):</label>
		                                            <textarea class="form-control <?=($config[$com][$type][$dfgallery][$val]['noidung_cke_photo'])?'form-control-ckeditor':''?>" name="dataMulti[<?=$i?>][noidung<?=$k?>]" id="noidung<?=$k?><?=$i?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode($item['noidung'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
